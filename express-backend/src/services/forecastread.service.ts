...
export const getTrend = async (query: ForecastQuery) => {
  ...
  const changeOver = (window: number) => {
    if (days.length < window || baseline === 0) return null;
    const later = mean(days.slice(Math.max(0, window - 7), window).map((day) => day.p50));
    ...
    items: forecast
      .map((row) => {
        ...
        return {
          ...
          averageDailyDemand: horizonDays === 0 ? null : round(total / horizonDays),
        };
      })
